...
import { Home, Store, Settings, Clock, BarChart2, ShieldCheck } from "lucide-react";
...
export default function BottomNav() {
  ...
  const navItems = role === "superadmin"
    ? [...baseNavItems, { to: "/superadmin", icon: ShieldCheck, label: "SA" }]
    : baseNavItems;
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-white shadow-card" : "text-muted-foreground"
              )}
            >
              <Icon className={cn("w-5 h-5", active ? "text-primary" : "text-sub")} />
              <span className={cn("text-[10px] font-bold", active ? "text-primary" : "text-sub")}>
                {label}
              </span>
            </Link>
          );
