...
type Company = { id: string; name: string; plan: string; active: boolean; created_at: string; };
...
export default function Superadmin() {
  ...
  const [form, setForm] = useState({ companyName: "", plan: "starter", adminName: "", adminEmail: "" });
  ...
  async function toggleActive(id: string, active: boolean) {
    await supabase.from("companies").update({ active: !active } as any).eq("id", id);
    await load();
  }

  async function changePlan(id: string, plan: string) {
    await supabase.from("companies").update({ plan } as any).eq("id", id);
    await load();
  }
  ...
  async function createCompanyAndAdmin() {
    ...
    try {
      const { data: company, error: compErr } = await supabase
        .from("companies").insert({ name: form.companyName, plan: form.plan, active: true } as any).select().single();

      if (compErr || !company) { alert("Erro ao criar empresa: " + compErr?.message); setCreating(false); return; }

      const { data: session } = await supabase.auth.getSession();
      const token = session.session?.access_token;

      const res = await fetch(`${SUPABASE_URL}/functions/v1/create-company-admin`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ company_id: (company as any).id, admin_name: form.adminName, admin_email: form.adminEmail }),
      });

      const result = await res.json();
      if (!res.ok || result.error) {
        alert("Empresa criada. Erro ao criar admin: " + (result.error || "Tente novamente."));
        await load(); setShowForm(false); setCreating(false); return;
      }
  ...
  return (
    <AdminLayout title="Superadmin" subtitle="Painel global da plataforma">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-background rounded-3xl w-full max-w-md p-6 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs font-bold text-sub">Dados do Admin</p>
              <div>
                <label className="text-xs font-bold text-sub mb-1 block">Nome</label>
                <input value={form.adminName} onChange={e => setForm(f => ({ ...f, adminName: e.target.value }))}
                  placeholder="Nome completo"
                  className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-xs font-bold text-sub mb-1 block">E-mail</label>
            ...
            <button onClick={createCompanyAndAdmin}
              ...
              {creating ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Criando...</> : <><Plus className="w-4 h-4" /> Criar e Gerar Acesso</>}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-3xl w-full max-w-md p-6 shadow-xl">
            <div className="text-center mb-5">
              ...
              <p className="text-xs text-sub mt-1">Copie as credenciais abaixo.</p>
            </div>
            <div className="space-y-3">
              {[{ label: "E-mail", value: credentials.email, key: "email" }, { label: "Senha", value: credentials.password, key: "pass" }].map(({ label, value, key }) => (
                <div key={key} className="bg-muted rounded-xl p-3">
                  <div className="text-xs font-bold text-sub mb-1">{label}</div>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-mono font-bold text-textMain break-all">{value}</span>
                    <button onClick={() => copyText(value, key)} className="shrink-0 p-1.5 rounded-lg bg-white shadow-sm">
                      {copied === key ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5 text-sub" />}
                    </button>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setCredentials(null)} className="mt-5 w-full bg-primary text-primary-foreground py-3 rounded-xl font-bold text-sm">
              Fechar
            </button>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {companies.length === 0 && <div className="text-center py-12 text-sub text-sm">Nenhuma empresa ainda.</div>}
          {companies.map(c => (
